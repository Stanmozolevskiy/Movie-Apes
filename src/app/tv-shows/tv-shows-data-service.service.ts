...
export class TvShowsDataServiceService {
  ...
  getVideos(subject: string, id: number):Observable<MovieResponce>{
    return this.HttpClient.get<MovieResponce>(`${this.BaseUrl}/${subject}/${id}/videos${this.key}`);
  };
  ...
  getAll(page:number=1, subject: string="tv", sort: string='&sort_by=popularity.desc'): Observable<MovieResponce> {
